...
Hermes also includes a read-only Microsoft Graph connector for calendar events,
mail messages, and contacts. The connector uses the same app-only credentials
described below and targets an explicit mailbox with `user_id`.
...
The bundled `msgraph` backend exposes these read-only tools:

| Tool                    | Purpose                                                                   |
| ----------------------- | ------------------------------------------------------------------------- |
| `msgraph_calendar_view` | List calendar occurrences, exceptions, and single events in a time range. |
| `msgraph_get_event`     | Read one calendar event.                                                  |
| `msgraph_list_messages` | List mailbox messages with optional OData filters.                        |
| `msgraph_get_message`   | Read one message, including an optional body selection.                   |
| `msgraph_list_contacts` | List contacts, optionally filtering by email address.                     |
| `msgraph_get_contact`   | Read one contact.                                                         |

Because this connector uses app-only client-credentials authentication, every
tool requires a `user_id` mailbox ID or UPN. The connector intentionally does
not use `/me`, which is only valid for delegated user tokens. Continue to apply
an Exchange application access policy to restrict the mailboxes the app may
read.

The connector follows Graph's opaque `@odata.nextLink` values for pagination.
It also sends `Prefer: outlook.timezone` for calendar responses and
`Prefer: outlook.body-content-type="text"` for message reads.

### Bitwarden Secrets Manager

Hermes can fetch the Graph credentials from Bitwarden Secrets Manager. Create
these three secrets in the selected BSM project using the exact key names:
...
Configure the Hermes profile's `config.yaml`:
...
Keep only the Bitwarden machine-account bootstrap token in the profile's
`~/.hermes/.env`:

```dotenv
BWS_ACCESS_TOKEN=0.xxxxxxxxx
```
...
Hermes resolves the BSM key/value pairs into the active profile's secret scope.
The connector reads them through the scoped secret API, so it does not call
Bitwarden directly and does not require the client secret to be stored in the
`.env` file.

### Connector Tests

From a supported Python 3.11-3.13 environment with the project's virtualenv
active, run:

```bash
scripts/run_tests.sh tests/plugins/test_msgraph_plugin.py \
  tests/tools/test_microsoft_graph_auth.py \
  tests/tools/test_microsoft_graph_client.py
```

These tests mock Graph and Bitwarden-scoped values; they do not contact either
service.
